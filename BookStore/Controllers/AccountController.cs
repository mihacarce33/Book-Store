...
namespace BookStore.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult AddUserToRole(int page = 1, int pageSize = 10)
        {
            ...
            foreach (var user in allUsers)
            {
                ...
                userWithRolesList.Add(new UserWithRoles
                {
                    UserId = user.Id,
                    Email = user.Email,
                    CurrentRoles = string.Join(", ", roles) // Join roles as a comma-separated string
                });
            }
        ...
        [HttpPost]
        public ActionResult AddUserToRole(string userId, string newRole)
        {
            ...
            // Remove existing roles (if needed) and add the new role
            var currentRoles = UserManager.GetRoles(user.Id);
            if (!currentRoles.Contains(newRole))
            {
                // Optionally remove the old roles if only one role is allowed
